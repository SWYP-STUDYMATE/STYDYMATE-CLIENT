...
test.describe('메인', () => {
  test('메인 페이지 진입 및 핵심 섹션 노출', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.locator('div:has-text("안녕하세요")')).toBeVisible(); // GreetingCard
  });
});
...
test.describe('로그인 플로우', () => {
  ...
  test('네이버 콜백 메시지', async ({ page }) => {
    ...
    await expect(page.locator('text=/네이버 로그인 (처리 중|성공|실패)/')).toBeVisible();
  });

  test('약관 동의 필수 체크 전/후 완료 버튼 상태', async ({ context, page }) => {
  ...
  test('대시보드 카드/차트 렌더링 - API 스텁', async ({ page }) => {
    await page.route('**/api/v1/analytics/dashboard', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          data: {
            overview: { last24h: { count: 123, avgDuration: 123, errorRate: 0.01, p50Duration: 50, p95Duration: 120, p99Duration: 250, avgCpuTime: 30 } },
...
test.describe('채팅', () => {
  test('채팅 페이지 로드 및 검색/빈상태 확인', async ({ authenticatedPage }) => {
...
test.describe('매칭', () => {
  test.fixme('매칭 프로필/목록 라우트가 App 라우터에 추가되면 구현', async ({ authenticatedPage }) => {
...
test.describe('프로필', () => {
  test('프로필 페이지 진입', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: '프로필' })).toBeVisible();
  });
});
...
test.describe('스케줄', () => {
  test('스케줄 페이지: 캘린더+리스트 일부 텍스트', async ({ authenticatedPage }) => {
...
test.describe('세션', () => {
  test('세션 리스트', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: '세션' })).toBeVisible();
    await expect(authenticatedPage.getByText(/예정된 세션|완료된 세션/)).toBeVisible();
  });

  test('세션 캘린더', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/sessions/calendar');
    await expect(authenticatedPage.getByRole('heading', { name: '세션 캘린더' })).toBeVisible();
  });

  test('세션 예약 생성', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/session/schedule/new');
    await expect(authenticatedPage.getByRole('heading', { name: '새 세션 예약' })).toBeVisible();
  ...
  test('오디오 연결 확인', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: '음성 세션 연결 확인' })).toBeVisible();
  ...
  test('비디오 연결 확인', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: '화상 세션 연결 확인' })).toBeVisible();
    await expect(authenticatedPage.getByText('화상 세션 시작하기')).toBeVisible();
  });

  test.fixme('오디오/비디오 룸(실제 WebRTC 세션) - 시뮬레이션/Mock 필요', async ({ authenticatedPage }) => {
...
test.describe('레벨 테스트(진입만)', () => {
  test('인트로/시작 버튼 존재', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByRole('heading', { name: 'AI 레벨 테스트' })).toBeVisible();
    await expect(authenticatedPage.getByRole('button', { name: '시작하기' })).toBeVisible();
  });
});
...
test.describe('온보딩 - 정보', () => {
  test('1단계: 영어 이름 입력 시 버튼 활성화', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/onboarding-info/1');
    await authenticatedPage.getByPlaceholder('닉네임을 입력해주세요').fill('MinHan');
    await expect(authenticatedPage.getByRole('button', { name: '다음' })).toBeEnabled();
  });
  ...
  test('3단계: 프로필 사진 업로드 UI 노출', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByText('프로필 사진을 업로드 해주세요.')).toBeVisible();
    await expect(authenticatedPage.getByText(/사진 선택|카메라로 사진 찍기/)).toBeVisible();
  });

  test('4단계: 자기소개 입력 시 다음 버튼 활성화', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/onboarding-info/4');
    await authenticatedPage.getByPlaceholder(/안녕하세요 현재 영어/).fill('소개 텍스트');
    await expect(authenticatedPage.getByRole('button', { name: '다음' })).toBeEnabled();
  });
...
test.describe('온보딩 - 언어', () => {
  test('모국어 선택 단계(Mock)', async ({ page }) => {
    await page.route('**/onboard/language/languages', async route => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([
...
test.describe('비디오 컨트롤 데모', () => {
  test('데모 페이지 로드', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByText('VideoControls Component Demo')).toBeVisible();
  });
});