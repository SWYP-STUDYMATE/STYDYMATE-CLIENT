...
### ⚠️ 절대 원칙
1. **코드 변경 = 문서 업데이트 필수**
   - API 엔드포인트 변경 → 서버의 API 문서 업데이트
   - 컴포넌트 props 변경 → 클라이언트 컴포넌트 문서 업데이트
   - 에러 코드 추가 → 서버의 에러 처리 문서 업데이트

2. **상호 참조 확인 의무**
   - 클라이언트 개발 시 → 서버 소스코드 및 문서 확인 필수
   - 서버 개발 시 → 클라이언트 컴포넌트 요구사항 확인 필수
   - Worker 개발 시 → 양쪽 프로젝트 연동 문서 확인 필수

3. **문서 동기화 확인**
   - TypeScript 인터페이스 ↔ 서버 DTO 일치
...
# 3. 문서 업데이트 (필수!)
# - API 변경시: ../../STUDYMATE-SERVER/docs/04-api/
...
### 🔗 크로스 프로젝트 확인 체크리스트
- [ ] 서버 API 변경 시 클라이언트 TypeScript 인터페이스 확인
- [ ] 클라이언트 컴포넌트 변경 시 서버 DTO 응답 형식 확인  
- [ ] 에러 처리 변경 시 양쪽 프로젝트 에러 코드 동기화
- [ ] WebSocket 이벤트 변경 시 송수신 로직 양쪽 확인
- [ ] 상태 관리 변경 시 서버 상태와 클라이언트 store 동기화
